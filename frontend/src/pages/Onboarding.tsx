...
const STEPS = ["Target Roles", "Skills", "Interests & Location", "AI Suggestions"];
...
export default function Onboarding() {
  ...
  // Read pre-populated data passed from CV upload via navigation state
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      const { data } = await api.post("/onboarding/suggest-skills", {
        skills: form.skills,
  ...
  const addSuggestedSkill = (skill: string) => {
    ...
    setSuggestedSkills((s) => s.filter((x) => x !== skill));
  };
  ...
  return (
    ...
    <div className="w-full max-w-2xl space-y-8">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 space-y-6">
        ...
        {step === 1 && (
          <StepPanel title="What are your current skills?" subtitle="Add all the technologies, tools, and competencies you have">
            ...
            {form.skills.length > 0 && (
              <p className="text-xs text-gray-500">{form.skills.length} skill{form.skills.length !== 1 ? "s" : ""} added. Hit Next to get AI suggestions.</p>
            )}
          </StepPanel>
        )}
        ...
        {step === 3 && (
          <StepPanel title="AI-suggested skills" subtitle="Based on your profile, here are skills that could strengthen your applications">
            <SkillSuggestions
              loading={loadingSuggestions}
              suggestions={suggestedSkills}
              currentSkills={form.skills}
              onAdd={addSuggestedSkill}
            />
            {form.skills.length > 0 && (
              <div className="mt-4 space-y-2">
                <p className="text-sm text-gray-400 font-medium">Your final skill set ({form.skills.length})</p>
                <div className="flex flex-wrap gap-2">
                  {form.skills.map((s) => (
                    <span key={s} className="bg-blue-900/40 border border-blue-700 text-blue-300 text-xs px-3 py-1 rounded-full">
                      {s}
                    </span>
                  ))}
                </div>
